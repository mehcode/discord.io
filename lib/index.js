...
var isNode = false; // typeof(window) === "undefined" && typeof(navigator) === "undefined";
...
function startConnection(client, gateway) {
	...
	client._ws.addEventListener('close', handleWSClose.bind(client));
	client._ws.addEventListener('error', handleWSClose.bind(client));
	client._ws.addEventListener('message', handleWSMessage.bind(client));
}
...
function handleWSMessage(event, flags) {
  var data = resolveEvent(event);
...
function AudioCB(voiceSession, audioChannels, encoder, maxStreamSize) {
	...
	if (maxStreamSize && !Opus) Opus = null; // require('cjopus');
...
ACBP.playAudioFile = function(location, callback) {
	if (!this._mixedDecoder) {
		if (!Opus) Opus = null;//require('cjopus');
...
/* Wrappers */
var _Emitter = require("eventemitter2");
function Emitter() {
  ...
  this._emitter = new _Emitter();
	this.on = function(eName, eFunc) {
		return this._emitter.on(eName, eOn);
	...
	this.once = function(eName, eFunc) {
		return this._emitter.once(eName, eOnce);

		function eOnce(e) {
			eFunc.apply(null, resolveEvent(e));
		}
	};
	this.removeListener = function(eName) {
    this._emitter.off(eName);
	};
	this.emit = function(eName) {
    this._emitter.emit(eName, {'detail': Array.prototype.slice.call(arguments, 1) });
	};
	return this;
}
